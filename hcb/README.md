...
| 文件 | 说明 |
|---|---|
| `HCB_Engine_RE_Report.docx` | 逆向分析/使用说明文档 |
| `hcb_extract.py` | 文本或资源提取脚本；hcb_extract.py — HCB脚本文本提取工具 |
| `hcb_inject.py` | 文本注入脚本；hcb_inject.py — HCB脚本文本注入工具 |
...
```bash
python hcb_extract.py <原始脚本或目录> <导出json或目录>
# 翻译/修改导出的 JSON
python hcb_inject.py <原始脚本或目录> <翻译json或目录> <输出脚本或目录>
```
