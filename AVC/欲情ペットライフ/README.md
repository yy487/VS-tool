...
| 文件 | 说明 |
|---|---|
| `avc_codec.py` | 编解码、压缩或加密辅助模块；AVC 引擎 (Adv.exe / SETSUEI 系列) 归档格式 codec |
| `avc_extract.py` | 文本或资源提取脚本；AVC 引擎归档解包工具 |
| `avc_inject.py` | 文本注入脚本；AVC 引擎归档封包工具 |
...
```bash
python avc_extract.py <原始脚本或目录> <导出json或目录>
# 翻译/修改导出的 JSON
python avc_inject.py <原始脚本或目录> <翻译json或目录> <输出脚本或目录>
```
