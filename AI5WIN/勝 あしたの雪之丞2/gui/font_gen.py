...
def calc_baseline_y(font, glyph_h: int, ref_char: str = '国') -> int:
    """Return a stable draw.text y for this font and glyph height.

    Important: punctuation must not be vertically centered independently.
    We align every glyph to a shared baseline derived from a representative CJK glyph,
    then only center X per character. This keeps commas, periods, brackets and wave
    dashes at their natural vertical positions relative to the font baseline.
    """
    bbox = font.getbbox(ref_char)
    if not bbox:
        return 0
    ref_h = bbox[3] - bbox[1]
    return (glyph_h - ref_h) // 2 - bbox[1]


def render_glyph(ch: str, font, glyph_w: int, glyph_h: int, *,
                 baseline_y: int | None = None, mask_mode: str = 'clear') -> tuple[bytes, bytes]:
    ...
    # X: per-glyph visual centering.
    # Y: shared baseline, not per-glyph vertical centering.
    # Per-glyph Y centering breaks punctuation because marks such as ，。！？～ are
    # lifted into the middle of the 26x26 cell.
    cw = bbox[2] - bbox[0]
    x = (glyph_w - cw) // 2 - bbox[0]
    y = calc_baseline_y(font, glyph_h) if baseline_y is None else baseline_y
...
def build_font_banks(map_path: str, font_path: str, orig_dir: str, out_dir: str,
    ...
    manifest = {
        ...
        'notes': [
            ...
            'Generated glyphs use a shared baseline from ref char 国: X is centered per glyph, Y is not centered per glyph.',
        ],
    }
    ...
    for bank_name in banks:
        ...
        for ent in entries:
            ...
            if key not in rendered_cache:
                rendered_cache[key] = render_glyph(
                    target, font, bank.glyph_w, bank.glyph_h,
                    baseline_y=baseline_cache.setdefault(bank.glyph_h, calc_baseline_y(font, bank.glyph_h)),
                    mask_mode=mask_mode,
                )
        ...
        if bank_name != 'FONTHAN':
            for ch in direct_chars:
                ...
                if key not in rendered_cache:
                    rendered_cache[key] = render_glyph(
                        ch, font, bank.glyph_w, bank.glyph_h,
                        baseline_y=baseline_cache.setdefault(bank.glyph_h, calc_baseline_y(font, bank.glyph_h)),
                        mask_mode=mask_mode,
                    )
                fnt_g, msk_g = rendered_cache[key]
