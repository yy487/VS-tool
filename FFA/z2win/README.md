...
| 文件 | 说明 |
|---|---|
| `so4_extract.py` | 文本或资源提取脚本；Z2WIN SO4 文本提取工具 (FFA/G-SYS engine) |
| `so4_inject.py` | 文本注入脚本；Z2WIN SO4 文本注入工具 (FFA/G-SYS engine) |
...
```bash
python so4_extract.py <原始脚本或目录> <导出json或目录>
# 翻译/修改导出的 JSON
python so4_inject.py <原始脚本或目录> <翻译json或目录> <输出脚本或目录>
```
