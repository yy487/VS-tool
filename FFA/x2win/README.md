...
| 文件 | 说明 |
|---|---|
| `so4_lzss.py` | 编解码、压缩或加密辅助模块；FFA 引擎 LZSS 压缩/解压工具 (纯 Python 实现) |
| `x2win_so4_extract.py` | 文本或资源提取脚本；X2WIN SO4 文本提取工具 (FFA/G-SYS engine, 天巫女姫) |
| `x2win_so4_inject.py` | 文本注入脚本；X2WIN SO4 文本注入工具 (FFA/G-SYS engine, 天巫女姫) |
...
```bash
python x2win_so4_extract.py <原始脚本或目录> <导出json或目录>
# 翻译/修改导出的 JSON
python x2win_so4_inject.py <原始脚本或目录> <翻译json或目录> <输出脚本或目录>
```
