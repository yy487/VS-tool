...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input", type=Path, help="Input .scr file or directory in batch mode")
    parser.add_argument("output", type=Path, help="Output JSON file or directory in batch mode")
    parser.add_argument("--batch", action="store_true", help="Recursively probe .scr files.")
...
def main() -> int:
    ...
    if not args.batch:
        ...
        print(f"[ok] scr probe: {args.input} -> {args.output}")
        return 0
    ...
    for input_file in sorted(args.input.rglob("*.scr")):
        ...
        try:
            ...
            print(f"[ok] scr probe: {input_file} -> {output_file}")
            ...
            if not args.skip_errors:
                raise
    (args.output / "_batch_scr_probe_manifest.json").write_text(json.dumps({"format": "TE_V2_BATCH_SCR_PROBE", "ok": ok, "failed": failed, "items": items}, ensure_ascii=False, indent=2), encoding="utf-8")
