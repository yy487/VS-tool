...
| 文件 | 说明 |
|---|---|
| `fpk_tool (1).py` | 封包解析/解包/重打包工具；FPK 引擎封包工具 (TBitchX / 自研引擎) |
| `gsx1_extract.py` | 文本或资源提取脚本；GSX1 (.fbx) 文本提取器 — ペンギンワークス Love Caba 引擎 |
| `gsx1_inject.py` | 文本注入脚本；GSX1 (.fbx) 文本注入器 — ペンギンワークス Love Caba 引擎 |
...
```bash
python gsx1_extract.py <原始脚本或目录> <导出json或目录>
# 翻译/修改导出的 JSON
python gsx1_inject.py <原始脚本或目录> <翻译json或目录> <输出脚本或目录>
```

建议先用未修改的 JSON 做一次 round-trip：提取后立刻注入，并比对输出与原文件是否一致。

如需先处理封包/归档文件，可查看这些脚本的参数：`fpk_tool (1).py`。
